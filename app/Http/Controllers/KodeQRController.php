...
class KodeQRController extends Controller
{
    ...
    public function index()
    {
        try {
            ...
            foreach($kodeqr as $item){
                $devices = FormIsian::with(['jenis_form'])->where('qr_code',$item->uuid)->get();
                if($devices->isEmpty()){
                    $devices = null;
                }
    ...
    public function show($id)
    {
        try
        {
            ...
            $devices = FormIsian::with(['jenis_form'])->where('qr_code',$kodeqr->uuid)->pluck('uuid')->toArray();
            if(empty($devices)){
                $devices = null;
            }
    ...
    public function getKodeQR()
    {
        try 
        {
            ...
            $dataAll = FormIsian::select('uuid')->where('qr_code',$kodeqr)->get();
            if ($dataAll->isEmpty()) {
                return response()->json([
                    'success' => false,
                    'message' => 'Data Tidak Ditemukan',
